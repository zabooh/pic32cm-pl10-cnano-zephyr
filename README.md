...
- [Deep dives — how it works](#deep-dives--how-it-works) → [`DEEPDIVE.md`](DEEPDIVE.md)
...
- Want to understand *how it works* under the hood (drivers, interrupts, power, peripheral
  support, other PL10 parts)? → [`DEEPDIVE.md`](DEEPDIVE.md)
...
The step-by-step build log lives in `RUNBOOK.md`; the longer *how it actually works*
essays live in **[`DEEPDIVE.md`](DEEPDIVE.md)**, kept out of this README so the front page
stays about getting the board running. What's in there:

- **[Peripheral support status](DEEPDIVE.md#peripheral-support-status)** — what Zephyr
  drives on the PL10 today (clock/GPIO/UART ready; SPI/I²C/DMA a config step away;
  ADC/PWM/DAC/etc. need bridging), as a snapshot of the pinned revision.
- **[Bridging a peripheral before Zephyr supports it](DEEPDIVE.md#bridging-a-peripheral-before-zephyr-supports-it)**
  — the stopgap-driver pattern behind `pl10_adc.c`, and how to swap in the real Zephyr
  driver later in one file.
- **[Interrupts and the system tick](DEEPDIVE.md#interrupts-and-the-system-tick)** — the
  whole interrupt surface (UART + SysTick), tickless timing, round-robin slicing.
- **[The driver model behind a pin toggle](DEEPDIVE.md#the-driver-model-behind-a-pin-toggle)**
  — what `gpio_pin_toggle_dt()` actually compiles to, and the one indirection that survives.
...
- **[Porting to another PL10 device](DEEPDIVE.md#porting-to-another-pl10-device)** — effort
  by target (64 KB easy, 128 KB a small dtsi, 32 KB a RAM diet), and the mechanical changes.
