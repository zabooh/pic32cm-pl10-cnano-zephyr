...
There is no test suite in this repo — verification is build success + flash success + manual/`pyserial`-scripted command interaction over the board's virtual COM port at 115200 baud (`led on/off/toggle/blink <ms>`, `adc read`, `adc stream start/stop`, `reset`, `help`; a `pl10:~$ ` prompt and boot banner — see Architecture).
...
- **`cmd_parser.c`** — the console: its own thread (`cmd_tid`) running a hand-rolled line editor over `console_getchar()` (char-by-char) with echo, end-of-line backspace, and bash-style Up/Down history recall (5-deep ring buffer); dispatches lines via `strcmp`/`strncmp` to the `led`/`adc`/`reset`/`help` handlers. Prints the `pl10:~$ ` prompt and, once at startup, a boot banner with the build timestamp (`__DATE__`/`__TIME__`).

**Five threads total:** `main` + `idle` (kernel baseline) + `blink_tid` (led_ctrl.c) + `adc_stream_tid` (pl10_adc.c) + `cmd_tid` (cmd_parser.c). All three app threads are `K_THREAD_DEFINE` (auto-started at boot), priority 7.
...
Current usage: **FLASH 28.48% (17496/61440 B), RAM 52.05% (4264/8192 B)** — re-check with `ram_report`/`rom_report` after any change, this part has little headroom to spare. Most of the RAM growth from the original 33.50% baseline is the three app threads (per-thread cost ~376-760 B: a 256/320/640 B stack + a 120 B thread control block each, measured directly via `ram_report`). Budget for ~a dozen more small threads before RAM becomes the constraint.
