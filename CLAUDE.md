...
This is not a general-purpose codebase — almost everything under `C:\zw` besides `app/` and the dotfiles is third-party (Zephyr source, HAL modules, venv). Only `app/`, `tools/`, `RUNBOOK.md`, `README.md`, `reproduce-install.ps1`, `requirements-lock.txt`, and `.vscode/` are hand-maintained.

**`tools/faultloc.py`** — a fault post-mortem helper. Copy a Cortex-M fault dump from the terminal (the `***** HARD FAULT *****` / `r14/lr` / `Faulting instruction address` block), run `python tools/faultloc.py`, and it generates a disassembly listing (`build/zephyr/zephyr.lst`) and resolves the faulting PC and the LR return address to function + `file:line` (with source snippets and the exact faulting instruction). Reads the dump from the clipboard (PowerShell `Get-Clipboard`, stdlib only) or `--file`. Since app functions are `static` and get inlined at `-Os`, it uses `addr2line -i` to unwind the inline chain (e.g. `cmd_mem` → `handle_line` → `cmd_thread_entry`).
...
west flash
```
If `west flash` fails with `SWD/JTAG communication failure (FAULT ACK)`, the core is asleep / the DAP is in a leftover state, **or** a `pyocd gdbserver` from a debug session still holds the probe (only one pyOCD can hold it at a time — stop any debug session / GDB-server task first). Reset once and retry: `pyocd reset -t pic32cm6408pl10048 -f 100000` (from the build dir), then `west flash`. The VS Code *Zephyr: Flash* task bakes this in — it runs `pyocd reset` and `west flash` sequentially in one terminal (see README.md "Working in VS Code").
...
**Debugging this board is fault-prone unless you reset first.** The SWD port becomes intermittently inaccessible while the Cortex-M0+ core is asleep (`k_msleep`/timer-driven idle triggers WFI) — polling status while attached to an already-running, already-sleeping target reproducibly causes `SWD/JTAG communication failure (FAULT ACK)`, in both Cortex-Debug and plain `gdb`. Fix: always issue `monitor reset halt` immediately after connecting/attaching, before setting breakpoints or continuing — both attach-type `launch.json` configs do this automatically via `postAttachCommands`. **The same FAULT ACK hits `west flash`** — the flash connect can't halt a sleeping core, or a leftover `pyocd gdbserver` from a debug session still holds the probe (only one pyOCD can hold it). The VS Code *Zephyr: Flash* task pre-resets the target in the same terminal to avoid it; from the CLI, run `pyocd reset -t pic32cm6408pl10048 -f 100000` then `west flash`. A breakpoint on `while (1) {` (or a bare `break main`) also only fires once, at first loop entry — break on a line *inside* the loop body instead. Full detail: `RUNBOOK.md` Step 11c/11e.
